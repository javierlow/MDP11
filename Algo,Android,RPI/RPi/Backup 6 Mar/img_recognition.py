...
#for debugging only import matplotlib
class ImageProcessor():
    ...
    def sequence_images(self,listener_endpoint_pc):
        ...
        while 1:
            ...
            #if(end-start < 0.1):
            #   time.sleep(0.1-(end-start)) print("Camera too fast: {}. Slept for: 
            #   {}".format(end-start,0.1-(end-start)))
    ...
    def identify(self, listener_endpoint_rpi):
        ...
        reference_img = cv.imread('/home/pi/Desktop/u.jpg'.format(sys.path[0]), cv.IMREAD_GRAYSCALE)
    ...
    def getImageLocation(self,reference_contours, captured_image_location):
        ...
        for (i, c) in enumerate(captured_cnts):
            ...
            if(not(6 <= len(approx) <= 8 )):
                continue
            # conditional check if contour matches any of the conditions we are looking for, we append the 
            # distance and location condition: 6-8 edges; matches given shape up to 0.25 likeliness; object to 
            # image area ratio
            if (cv.matchShapes(reference_contours[0], c, 1, 0.0) < 0.1):
                ...
                if 0.105 < objectAreaRatio < 0.31:
                    # find X-axis of contour M = cv.moments(c) cx = int(M["m10"] / M["m00"]) find which part of 
                    #the image the contour is on if imgX / 3 < cx < 2 * (imgX / 3):
                    #    a = "center" elif cx < imgX / 3: a = "left" else: a = "right"
