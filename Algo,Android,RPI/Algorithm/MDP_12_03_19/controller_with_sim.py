...
class SimController:
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    self._gui.update_cells(updated_cells)
                    self._update_android()
                        ...
                        while True:
                            ...
                            if updated_or_moved == "updated" or updated_or_moved == "moved":
                                ...
                                if updated_or_moved == "updated":
                                    ...
                                    updated_cells = self._gui.move_robot(value)
                                    self._gui.update_cells(updated_cells)
                            else:
                                break
    ...
    def _auto_calibrate(self):
        ...
        if y >= 17 and x <= 2:
            ...
            if self._robot.facing == SOUTH:
                ...
                self._robot.turn_robot(self._sender, LEFT)
                self._last_calibrate = 0
        if self._last_calibrate >= 4:
            y, x = get_coordinates(self._robot.center)

            if x <= 2:
                if self._robot.facing == NORTH:
                    if self._check_left_possible():
                        self._wall_calibration()
                        self._last_calibrate = 0
                if self._robot.facing == WEST:
                    if self._check_front_possible():
                        self._robot.turn_robot(self._sender, RIGHT)
                        self._wall_calibration()
                        self._robot.turn_robot(self._sender, LEFT)
                        self._last_calibrate = 0
    ...
    def _check_front_possible(self):
        y, x = get_coordinates(self._robot.center)
        if self._robot.facing == NORTH:
            if self._robot.discovered_map[y+1][x-1] == self._robot.discovered_map[y+1][x+1]:
                if y == 18 or self._robot.discovered_map[y][x] == 1:
                    return True
                else:
                    if self._robot.discovered_map[y+2][x-1] == self._robot.discovered_map[y+2][x+1]:
                        return True
                    else:
                        return False
            else:
            ...
            if self._robot.discovered_map[y-1][x-1] == self._robot.discovered_map[y+1][x-1]:
                if x == 1 or self._robot.discovered_map[y][x] == 1:
                    return True
                else:
                    if self._robot.discovered_map[y-1][x-2] == self._robot.discovered_map[y+1][x-2]:
                        return True
                    else:
                        return False
            else:
            ...
            if self._robot.discovered_map[y-1][x-1] == self._robot.discovered_map[y-1][x+1]:
                if y == 1 or self._robot.discovered_map[y][x] == 1:
                    return True
                else:
                    if self._robot.discovered_map[y-2][x-1] == self._robot.discovered_map[y-2][x+1]:
                        return True
            ...
            if self._robot.discovered_map[y-1][x+1] == self._robot.discovered_map[y+1][x+1]:
                if x == 13 or self._robot.discovered_map[y][x] == 1:
                    return True
                else:
                    if self._robot.discovered_map[y-1][x+2] == self._robot.discovered_map[y+1][x+2]:
                        return True
